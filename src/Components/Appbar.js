...
const Header = () => {
  ...
  useEffect(() => {
    // Fetch winnings and show notification if there are any
    axios
      .get('http://localhost:8000/winners/all')
      .then((response) => {
        const userWinnings = response.data.filter(
          (winning) => winning.product.user.uid === parseInt(userId, 10)
        );

        if (userWinnings.length > 0) {
          const latestWinning = userWinnings[userWinnings.length]; // Get the latest win
          setNotification(`Congratulations! You won the auction for ${latestWinning.product.name}`);
